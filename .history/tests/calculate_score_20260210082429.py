...
def get_current_version():
    """Leest de versie uit environment of direct uit het VERSION bestand."""
    # 1. Probeer eerst de environment variable (vanuit de pipeline)
    version = os.getenv("APP_VERSION")
    if version:
        return version
    
    # 2. Fallback: Lees direct uit het bestand op je Mac
    try:
        with open("VERSION", "r") as f:
            return f.read().strip()
    except:
        return "2.2.0" # De allerlaatste fallback
...
def calculate():
    pillars = [
        ("Entry Check", "entry"),       # Nieuw: Basis connectiviteit
        ("Code Quality", "sonar"),
